...
# indx <- match(historical.data$uniqueId, updated_encounters_result$uniqueId, nomatch = 0)
# historical.data$ENC_CLOSED_CHARGE_STATUS[indx != 0] <- updated_encounters_result$ENC_CLOSED_CHARGE_STATUS[indx]
# 
# indx <- match(historical.data$uniqueId, updated_encounters_result$uniqueId, nomatch = 0)
# historical.data$Y_ENC_COSIGN_TIME[indx != 0] <- updated_encounters_result$Y_ENC_COSIGN_TIME[indx]
# 
# indx <- match(historical.data$uniqueId, updated_encounters_result$uniqueId, nomatch = 0)
# historical.data$Y_ENC_CLOSE_TIME[indx != 0] <- updated_encounters_result$Y_ENC_CLOSE_TIME[indx]

saveRDS(historical.data, paste0(path,"historical_data_6-19-2022.rds"))
...
# min_date_util <- Sys.Date() - 61
# max_date_util <- Sys.Date() - 1
# utilization.data <- utilization.data %>% filter(Appt.DateYear >= min_date_util)
